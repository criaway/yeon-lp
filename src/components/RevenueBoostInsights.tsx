...
const RevenueBoostInsights: React.FC<RevenueBoostInsightsProps> = ({
  ...
  const generateInsights = () => {
    ...
    // Sort by streams
    const sortedByStreams = [...usedPlatforms].sort(
      (a, b) => b.streams - a.streams,
    );
    // Sort by revenue
    const sortedByRevenue = [...usedPlatforms].sort(
    ...
    if (sortedByStreams.length > 0 && sortedByRevenue.length > 0) {
      ...
      const highestRatePlatforms = platforms
        ...
        .slice(0, 2);
      if (
        highestRatePlatforms.length > 0 &&
        mostStreamedPlatform.platformInfo?.rate &&
        highestRatePlatforms[0].rate >
          mostStreamedPlatform.platformInfo.rate * 1.5
      ) {
        insights.push({
          id: "better-platforms",
